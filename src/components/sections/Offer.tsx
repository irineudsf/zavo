  ...
  return (
    <section style={{ background: '#F5B800', padding: '96px 24px' }}>
      <div style={{ maxWidth: 700, margin: '0 auto', textAlign: 'center' }}>
        ...
        <h2 style={{ fontSize: 'clamp(28px, 4vw, 44px)', fontWeight: 800, color: '#0D0D0D', letterSpacing: -1.5, lineHeight: 1.1, marginBottom: 32 }}>Tudo incluso. Você decide o preço.</h2>
        ...
        <div style={{ background: '#0D0D0D', borderRadius: 16, padding: '36px 40px', marginBottom: 32 }}>
          ...
          <div style={{ fontSize: 'clamp(32px, 6vw, 56px)', fontWeight: 900, color: '#F5B800', letterSpacing: -2, lineHeight: 1, marginBottom: 8 }}>Você decide.</div>
          <div style={{ fontSize: 16, color: 'rgba(255,255,255,0.6)', lineHeight: 1.5 }}>
            Acreditamos no nosso trabalho. Quando você ver o resultado, paga o que achar justo. Se não gostar — não cobra nada.
          </div>
        </div>
        <a href={WHATSAPP_URL} target="_blank" rel="noopener" style={{
          ...
          Quero meu site — eu decido o preço
        </a>
        <p style={{ fontSize: 15, color: 'rgba(0,0,0,0.5)', marginTop: 20, lineHeight: 1.6 }}>
          Sem contrato, sem pegadinha. Se não gostar — não paga.
        </p>
      </div>
    </section>
  )
